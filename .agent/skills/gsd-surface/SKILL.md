...
<objective>
Manage the runtime skill surface without reinstall. Reads/writes `.agent/.gsd-surface.json`
(sibling to `.agent/.gsd-profile`) and re-stages the active skills directory in place.
Skill dirs live at `.agent/skills/gsd-*/`.
...
(`.agent`), NOT the skills sub-directory (`.agent/skills`).

This matches `installRuntimeArtifacts` and `uninstallRuntimeArtifacts`, which also
receive `.agent` as `configDir`. The skill dirs themselves live at
`.agent/skills/gsd-*/` because the `claude global` layout has `destSubpath =
'skills'` — they are derived from `configDir`, not the root for it.

```bash
# Claude Code — global install
RUNTIME_CONFIG_DIR="${CLAUDE_CONFIG_DIR:-.agent}"
SCOPE="global"
...
<execution_context>
Surface state file: `.agent/.gsd-surface.json`
Install profile marker: `.agent/.gsd-profile`
Skill dirs: `.agent/skills/gsd-*/`
Engine module: `/Users/tecbusiness/Desktop/IMPORTANT/VMS-Bare/.agent/get-shit-done/bin/lib/surface.cjs`
Cluster definitions: `/Users/tecbusiness/Desktop/IMPORTANT/VMS-Bare/.agent/get-shit-done/bin/lib/clusters.cjs`
</execution_context>
